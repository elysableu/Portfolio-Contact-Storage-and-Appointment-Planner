...
function App() {
  ...
  */
  ...
  const addContacts = useCallback((name, phone, email) => {
    setContacts((prevContacts) => [...prevContacts, {name, phone, email}])
  }, []);

  const addAppointments = useCallback((title, contact, date, time) => {
    setAppointments((prevAppts) => [...prevAppts, {title, contact, date, time}])
  }, []);
